...
- **R5 (Sol, base main, on `a549fd9`):** NOT clean — 4 in-scope P1s, incl. a real env-proxy egress hole and a SECOND self-inflicted regression (R5-4, from R2-3). Handled at `91259e7`:
  - **Shipped (safe, non-behavioral):** R5-1 env-proxy egress (`trust_env=False` on client + broker httpx); R5-4 typed-config resolution (`RoleEntry.endpoint` + `_config_backends` reads `BackendEntry.url`). 424 pass, Ruff clean.
  - **Deferred to the hardening spec (change the wire contract / churn a broad test surface):** R5-2/H3 quarantine-aware `complete()` candidate selection (fix flips `model:"role:X"` live-resolve → client-resolved concrete model — a deliberate design decision); R5-3/H4 explicit-model `generate_json` served-model provenance (churns ~8 tests using a fixed response model).
- **RE-ESCALATED to Jason (2026-07-23).** 5 rounds, 23 findings, TWO self-inflicted regressions (R2-2, R5-4); the never_cloud/attribution/candidate surface is entangled with the client/broker wire contract + config typing. This is a deliberate design pass, not release-prep convergence. **Recommendation: pause the release convergence; the privacy core + env-proxy fix are a solid shipped foundation; move H1–H4 into a spec-gated hardening effort.** Awaiting Jason's call.
